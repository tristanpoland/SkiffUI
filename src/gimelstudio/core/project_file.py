...
class ProjectFileIO(object):
    ...
    def CreateNodesFromData(self, nodegraph):
        try:
            ...
            wx.MessageBox(message=str.join("Unable to create graph from file, is the file corrupted? (Details: check 1 failure:)", repr(e)))
    ...
    def OpenFile(self, file_path):
        with open(file_path, "r") as file:
            try:
                ...
                wx.MessageBox(message=str.join("Unable to create graph from file, is the file corrupted? (Details: check 2 failure:)", repr(e)))
